...
function numtodeci( num ) {
	return ( Math.floor( num / 100 ) % 10 ).toString().padStart( 1, "0" );
}
...
const fifsecbtn = document.getElementById( "15s" );
const ffifsecbtn = document.getElementById( "45s" );
const music = new Audio( "bell01.mp3" );
// function saynow() {
...
function tick() {
	now = Math.max( giventime * 1000  - new Date().getTime() + startTime, 0 );
	// now = Math.max( 63 * 1000  - new Date().getTime() + startTime, 0 );
	...
	timer.textContent =  numtosecond( now )+":"+numtodeci(now);
	if ( now == 0 ) {
		reset();
		music.play();
	}
...
function fifsec() {
	reset();
	giventime = 15;
	timer.textContent = "15:0"
	// timer.textContent = "00:00:15"
}
function ffifsec() {
	reset();
	giventime = 45;
	timer.textContent = "45:0"
	// timer.textContent = "00:00:45"
...
fifsecbtn.onclick = fifsec;
ffifsecbtn.onclick = ffifsec;
